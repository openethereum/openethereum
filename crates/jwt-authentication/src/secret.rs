...
#[derive(Debug)]
pub struct ParseSecretError;

impl std::fmt::Display for ParseSecretError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        "provided string was not valid hex-encoded 256 bit secret key".fmt(f)
    }
}
...
impl FromStr for Secret {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let _ = hex::decode_to_slice(s.strip_prefix("0x").unwrap_or(&s), &mut secret)
            .map_err(|_| ParseSecretError)?;
        Ok(Self(secret))
    }
}
...
impl Secret {
    pub fn new(file_path: String, random: &dyn SecureRandom) -> anyhow::Result<Self> {
        if let Ok(data) = fs::read_to_string(&file_path) {
            info!("Reading jwt secret from file");
