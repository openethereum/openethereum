...
impl<K: Kind> VerificationQueue<K> {
	...
	pub fn import(&self, input: K::Input) -> Result<H256, (Option<K::Input>, Error)> {
		...
		{
			if self.processing.read().contains_key(&hash) {
				bail!((Some(input), ErrorKind::Import(ImportErrorKind::AlreadyQueued).into()));
			}

			let mut bad = self.verification.bad.lock();
			if bad.contains(&hash) || bad.contains(&raw_hash) {
				bail!((Some(input), ErrorKind::Import(ImportErrorKind::KnownBad).into()));
			}
		...
		match K::create(input, &*self.engine, self.verification.check_seal) {
			Ok(item) => {
				if self.processing.write().insert(hash, item.difficulty()).is_some() {
					bail!((None, ErrorKind::Import(ImportErrorKind::AlreadyQueued).into()));
				}
